...
namespace defibox {
    ...
    static std::pair<extended_symbol, extended_symbol> get_pair_symbols( const uint64_t pair_id, const symbol sort )
    {
        defibox::pairs _pairs( code, code.value );
        auto pairs = _pairs.get( pair_id, "AiSwapLibrary: INVALID_PAIR_ID" );
        eosio::check( pairs.token0.symbol == sort || pairs.token1.symbol == sort, "SwapLibrary: sort symbol doesn't match");
        ...
        return sort == pairs.token0.symbol ? 
        std::pair<extended_symbol, extended_symbol>{ token0, token1 } : 
        std::pair<extended_symbol, extended_symbol>{ token1, token0 };
    }
