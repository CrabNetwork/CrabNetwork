...
checksum256 hash_asset_ids(extended_symbol token0, extended_symbol token1) {
   std::vector <uint64_t> asset_ids = {
      token0.get_contract().value,
      token1.get_contract().value,
      token0.get_symbol().code().raw(),
      token1.get_symbol().code().raw(),
      token0.get_symbol().precision(),
      token1.get_symbol().precision()
   };
