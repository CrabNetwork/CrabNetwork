...
namespace defibox {
    ...
    const name code = "swap.box"_n;
    const name lp_code = "lptoken.box"_n;
    ...
    struct [[eosio::table]] pairs_row {
        ...
        uint128_t hash() const { return utils::uint128_hash(token0.to_string() + "-" + token1.to_string()); }
    };
    typedef eosio::multi_index< "pairs"_n, pairs_row,
     indexed_by < name("byhash"), const_mem_fun < pairs_row, uint128_t, &pairs_row::hash>>> pairs;
    ...
    {
        const auto supply = utils::get_supply(lp_token.get_extended_symbol());
    ...
    static std::pair<extended_symbol, extended_symbol> get_pair_extended_sym( const uint64_t pair_id, const symbol sort )
    {
        ...
        auto token0 = extended_symbol{pairs.token0.symbol, pairs.token0.contract};
        auto token1 = extended_symbol{pairs.token1.symbol, pairs.token1.contract};
        return sort == pairs.reserve0.symbol ?
            std::make_pair(token0, token1) :
            std::make_pair(token1, token0);
    }
    ...
    static extended_symbol get_out_extended_sym( const uint64_t pair_id, const symbol in_sym )
    {
        ...
        return in_sym == pairs.token0.symbol ? extended_symbol{pairs.token1.symbol, pairs.token1.contract} : extended_symbol{pairs.token0.symbol, pairs.token0.contract};
    }

    static asset get_amount_out( const uint64_t pair_id, const asset in_asset, const uint16_t fee = 30 )
    {
        const auto [ reserve_in, reserve_out ] = defibox::get_reserves( pair_id, in_asset.symbol );
    ...
    static uint128_t uint128_hash_tokens(token token0, token token1) {
        string hash = token0.to_string() + "-" + token1.to_string();
        return std::hash<string>{}(hash);
    }

    static uint64_t get_pairid_by_tokens(extended_symbol token0, extended_symbol token1) {
        auto tokenA = token{token0.get_contract(), token0.get_symbol()};
        auto tokenB = token{token1.get_contract(), token1.get_symbol()};
        defibox::pairs _pairs( code, code.value );
        auto pair_by_hash = _pairs.get_index<name("byhash")>();

        uint128_t hash_id0 = defibox::uint128_hash_tokens(tokenA, tokenB);
        auto p_itr0 = pair_by_hash.find(hash_id0);
        if(p_itr0 != pair_by_hash.end()) p_itr0->id;

        uint128_t hash_id1 = defibox::uint128_hash_tokens(tokenB, tokenA);
        auto p_itr1 = pair_by_hash.find(hash_id0);
        if(p_itr1 != pair_by_hash.end()) p_itr1->id;
